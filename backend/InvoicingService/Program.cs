...
// CREATE invoice
app.MapPost("/api/invoices", async (Invoice invoice, InvoicingDbContext db) => 
{
    var lastInvoice = await db.Invoices.OrderByDescending(i => i.SequentialNumber).FirstOrDefaultAsync();
    invoice.SequentialNumber = lastInvoice != null ? lastInvoice.SequentialNumber + 1 : 1;
...
// ADD item to invoice
app.MapPost("/api/invoices/{id}/items", async (int id, InvoiceItem item, InvoicingDbContext db, IStockService stockService) => 
{
...
// CLOSE invoice (print) - updates stock
